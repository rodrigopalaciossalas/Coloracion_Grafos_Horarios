...
with open('asignaciones_alumnos_toda_la_malla.json', 'r') as f:
...
try:
    with open('asignaciones_profesores_toda_la_malla.json', 'r') as f:
        professor_assignments = json.load(f)
except FileNotFoundError:
    print("Advertencia: 'asignaciones_profesores_toda_la_malla.json' no encontrado. Los conflictos de profesores no se considerarán.")
...
plt.figure(figsize=(18, 10)) # Aumentar el ancho para dar espacio a la leyenda en la derecha
...
# --- Dibujar las aristas con diferentes colores (y grosor uniforme) ---
# Reducir el grosor de la arista
uniform_width = 0.5
...
nx.draw_networkx_edges(G, pos, edgelist=student_only_edges, edge_color='skyblue', width=uniform_width, alpha=0.6)
nx.draw_networkx_edges(G, pos, edgelist=professor_edges, edge_color='red', width=uniform_width, alpha=0.6)
nx.draw_networkx_edges(G, pos, edgelist=shared_edges, edge_color='purple', width=uniform_width, alpha=0.7)

# Dibujar los nodos del grafo.
# Reducir el tamaño de los nodos
nx.draw_networkx_nodes(G, pos, node_color=[colors_map(coloring[node] - 1) for node in G.nodes()], node_size=800) # Reduced node_size
# Dibujar las etiquetas (nombres de los cursos) en los nodos.
# Reducir el tamaño de la fuente
nx.draw_networkx_labels(G, pos, font_size=5, font_weight='bold') # Reduced font_size
...
edge_legend_student = plt.Line2D([0], [0], color='skyblue', lw=uniform_width, label='Conflicto Alumno')
edge_legend_professor = plt.Line2D([0], [0], color='red', lw=uniform_width, label='Conflicto Profesor')
edge_legend_shared = plt.Line2D([0], [0], color='purple', lw=uniform_width, label='Conflicto Ambos')

# Añadir el número mínimo de aulas como un elemento de la leyenda
# Se crea un Line2D "fantasma" que solo contendrá el texto
aula_min_text = f'Aulas Requeridas: {max_simultaneous_exams}'
aula_min_patch = plt.Line2D([0], [0], color='w', marker='None', linestyle='None', label=aula_min_text)


all_patches = patches_colors + [edge_legend_student, edge_legend_professor, edge_legend_shared, aula_min_patch]

plt.legend(handles=all_patches, title="Leyenda", bbox_to_anchor=(1.02, 1), loc='upper left', ncol=1)

plt.title("Grafo de Conflictos con Cursos Coloreados por Franjas Horarias Asignadas")
plt.axis('off') # Ocultar los ejes
plt.tight_layout(rect=[0, 0, 0.85, 1]) # Ajustar el diseño para dar espacio a la leyenda a la derecha
plt.show()